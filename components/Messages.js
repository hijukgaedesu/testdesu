...
import htm from 'htm';
import { Send, Bot, ArrowLeft, MoreHorizontal, MessageSquare, Settings, Trash2 } from 'lucide-react';
import { getStoredMessages, saveMessage, deleteMessage } from '../services/storage.js';
import { getChatResponse } from '../services/gemini.js';

const html = htm.bind(React.createElement);

// Extracted Component: ConversationList
...
// Extracted Component: ChatWindow
const ChatWindow = ({ aiId, aiSettings, messages, inputText, setInputText, onSend, isTyping, onBackClick, userProfile, onDeleteMessage }) => {
    ...
    if (!aiId) {
        return html`
            <div className="hidden md:flex flex-1 flex-col items-center justify-center h-full text-center p-8 border-r border-gray-100 max-w-[600px]">
                <h2 className="text-3xl font-bold text-black mb-2">메시지 선택</h2>
                <p className="text-gray-500">기존 대화를 선택하거나 새로운 대화를 시작하세요.</p>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 border-r border-gray-100 max-w-[600px]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            ${currentConversation.map((msg) => html`
                <div key=${msg.id} className=${`flex group items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <!-- Left side controls (Delete for User) -->
                    ${msg.sender === 'user' && html`
                         <button 
                            onClick=${(e) => { e.stopPropagation(); onDeleteMessage(msg.id); }}
                            className="text-gray-300 hover:text-red-500 transition-colors p-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100"
                            title="삭제"
                        >
                            <${Trash2} size=${16} />
                        </button>
                    ...
                    <!-- Bubble -->
                    <div className=${`max-w-[70%] rounded-md px-4 py-3 text-[15px] whitespace-pre-wrap break-words ${
                        msg.sender === 'user' 
                     ...
                     <!-- Right side controls (Delete for AI) -->
                     ${msg.sender === 'ai' && html`
                         <button 
                            onClick=${(e) => { e.stopPropagation(); onDeleteMessage(msg.id); }}
                            className="text-gray-300 hover:text-red-500 transition-colors p-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100"
            ...
            ${isTyping && html`
                <div className="flex justify-start items-end gap-2">
                    ...
                    <div className="bg-[#eff3f4] rounded-md px-4 py-3 flex items-center gap-1">
